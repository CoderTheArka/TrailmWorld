...
const firebaseConfig = {
 ...
 function pic(name){
  localStorage.setItem(name,name);
  console.log(name);
  var ref1 = firebase.database().ref('Searched Box/').child(name);
  ref1.once('value',function(hfj){
  var data1 = hfj.val();
  var NAMEyk = data1.EMAIL;
  console.log(NAMEyk);
  var semail = NAMEyk
  var NAMEy   = semail.substring(0, semail.lastIndexOf("@"));
  var abc = firebase.database().ref('Image/').child(NAMEy);
  abc.once('value',function(info){
  var datak = info.val();
  var PhotoLink = datak.LINK  ;
  localStorage.setItem(name+'Hello',PhotoLink);
...
var ref1 = firebase.database().ref('Searched Box/').child(dd);
  ref1.once('value',function(hfj){
  var data1 = hfj.val();
  var NAMEyk = data1.EMAIL;
  console.log(NAMEyk);
  ...
  var datak = info.val();
  var PhotoLink = datak.LINK  ;
  document.getElementById('profile').src = PhotoLink;
});
 });
  ...
  function s() { 
    ...
    var semail = NAMEyk
    var NAMEy   = semail.substring(0, semail.lastIndexOf("@"));
    localStorage.setItem('ReciverPhotoName',NAMEy);

    setTimeout(function(){
      var ref12 = firebase.database().ref('Image/').child(localStorage.getItem('ReciverPhotoName'));
      ref12.once('value',function(hfjk){
      var data12 = hfjk.val();
      var NAMEyy = data12.LINK;
      localStorage.setItem('ReciverPhoto',NAMEyy);
...
function sendPhoto(e)
{
  ...
  input.accept = 'image/*';
 input.onchange = e => {
  var math_random = Math.floor(Math.random() * 100);
  console.log('working');
...
function sendPhoto65(e)
{
 ...
 input1.onchange = e => {
    ...
    var math_random = Math.floor(Math.random() * 1000);
    console.log('working');
    files = e.target.files;
    reader = new FileReader();
    reader.onload = function(){
